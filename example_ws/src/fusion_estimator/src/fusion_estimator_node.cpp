...
#include "base_interfaces_demo/msg/fusion_estimator_test.hpp" 
...
using base_interfaces_demo::msg::FusionEstimatorTest;
...
public:
    ...
    : Node("fusion_estimator_node")
    {
        Lowstate_subscriber = this->create_subscription<unitree_go::msg::LowState>(
            "lowstate", 10,  // 话题名称需确认实际使用值
            std::bind(&FusionEstimatorNode::LowStateCallback, this, std::placeholders::_1));
        ...
        FETest_publisher = this->create_publisher<FusionEstimatorTest>(
            "SMXFE/Estimation", 10);
...
private:
    ...
    rclcpp::Publisher<FusionEstimatorTest>::SharedPtr FETest_publisher;
    ...
    {
        // 保持原始处理逻辑，仅修改消息访问方式
        auto fusion_msg = FusionEstimatorTest();
        fusion_msg.stamp = this->get_clock()->now();

        // 修改字段访问方式（删除()运算符）
        for(int i=0; i<3; i++){
            fusion_msg.data_check_a[0+i] = msg->imu_state.accelerometer[i];
            fusion_msg.data_check_a[3+i] = msg->imu_state.rpy[i];
            fusion_msg.data_check_a[6+i] = msg->imu_state.gyroscope[i];
        }
        ...
        for(int LegNumber = 0; LegNumber<4; LegNumber++)
        {
            for(int i = 0; i < 3; i++)
            {
                fusion_msg.data_check_b[LegNumber*3+i] = msg->motor_state[LegNumber*3+i].q;
                fusion_msg.data_check_c[LegNumber*3+i] = msg->motor_state[LegNumber*3+i].dq;
            }
        }
        ...
        for(int i = 0; i < 3; i++)
        {
            LatestMessage[0][3*i+2] = msg->imu_state.accelerometer[i];
        }
        for(int i = 0; i < 9; i++)
        {
        ...
        for(int i = 0; i < 3; i++)
        {
            LatestMessage[1][3*i] = msg->imu_state.rpy[i];
        }
        for(int i = 0; i < 3; i++)
        {
            LatestMessage[1][3*i+1] = msg->imu_state.gyroscope[i];
        }
        for(int i = 0; i < 9; i++)
        {
        ...
        for(int LegNumber = 0; LegNumber<4; LegNumber++)
        {
            for(int i = 0; i < 3; i++)
            {
                LatestMessage[2][LegNumber*3+i] = msg->motor_state[LegNumber*3+i].q;
                LatestMessage[2][12+ LegNumber*3+i] = msg->motor_state[LegNumber*3+i].dq;
            }
            LatestMessage[2][24 + LegNumber] = msg->foot_force[LegNumber];
            fusion_msg.others[LegNumber] = msg->foot_force[LegNumber];
